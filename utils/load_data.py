...
def char_index_single(sent, sent_max_size):
    if isinstance(sent,float)==True:
        sent = '其'
    word2idx, idx2word = load_char_vocab()
    s_list= []
    sent = [word2idx[word.lower()] for word in sent if len(word.strip()) > 0 and word.lower() in word2idx.keys()]
    sent = sent[:sent_max_size] + [0] * (sent_max_size - len(sent))
    sent = np.reshape(sent, (1, 200))
    return sent
